...
export const SHIPMENT_MERGE_CODES: MergeCode[] = [
  ...
  { token: "{{ shipment.origin }}", label: "Port of Load" },
  { token: "{{ shipment.destination }}", label: "Port of Discharge" },
  ...
  { token: "{{ shipment.etd }}", label: "Departure date from Port of Load (ETD)" },
  { token: "{{ shipment.eta }}", label: "Arrival date at Port of Discharge (ETA)" },
