-- 0045  Editable qty on derived charge lines + OF-06 Ocean Freight (LCL-DDP)
--
-- A charge line whose qty is normally derived (weight/volume unit, or the
-- new OF-06 code) can now be hand-edited. The override is a per-line flag;
-- while it's set, save_quote stores the operator's number as-is instead of
-- the packing-list figure. OF-06 itself is a front-end catalog + calc rule
-- (qty = MAX(0.50, freight ton)); nothing DB-side is needed for it beyond
-- this flag.
--
-- Also (re)asserts packing_list_items.cbm from 0043 in case that migration
-- was never applied on this database.
--
-- Run in the Supabase SQL editor after 0044. Self-contained & idempotent.

alter table public.packing_list_items
  add column if not exists cbm numeric;

alter table public.quote_lines
  add column if not exists qty_override boolean not null default false;

-- ---- Re-emit save_quote so p_lines carries qty_override --------------------
-- (Unchanged from 0044 except the quote_lines insert reads qty_override.)
...
begin
  ...
  insert into public.quote_lines
    (quote_id, position, category, code, description, cur, unit, qty,
     qty_override, buy, margin, vat_pct, sell)
  select
    v_id,
