-- 0047  Editable percentage on the %-of-something fee lines (FW-01, DIS-01)
--
-- FW-01 Forwarding Fee (1% of International Freight) and DIS-01 Disbursement
-- Fee (2.5% of Customs VAT + Duty) default to those rates but the operator
-- sometimes raises or lowers them. `quote_lines.fee_rate` holds the override
-- percentage; NULL = use the code's default.
--
-- Run in the Supabase SQL editor after 0046. Self-contained & idempotent.

alter table public.quote_lines
  add column if not exists fee_rate numeric;

-- ---- Re-emit save_quote so p_lines carries fee_rate ----------------------
-- (Unchanged from 0045 except the quote_lines insert reads fee_rate.)
