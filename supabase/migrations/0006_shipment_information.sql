-- EXPAC Rate Desk 0006 — full Shipment Information field set. Run after 0005.
-- Adds commercial value, insurance amount, incoterms, and the sea + air
-- carrier/booking fields to quotes, and widens save_quote to match.

-- Safety net for a skipped earlier migration.
...
alter table public.quotes
  ...
  add column if not exists mawb_no          text,
  add column if not exists hawb_no          text,
  add column if not exists flight_no        text,
  add column if not exists flight_date      date,
  add column if not exists carrier_name     text;

-- Replace save_quote (old 15-arg signature from 0004 -> drop first).
...
begin
  if p_id is null then
    insert into public.quotes
      (reference, client_id, supplier_id, mode, commodity, origin, destination,
       delivery_terms, valid_until, status, commercial_value, insurance_amount,
       incoterms, vessel_name, mbl_no, hbl_no, container_no, etd, eta, mawb_no,
       hawb_no, flight_no, flight_date, carrier_name, fx_usd_zar, fx_cny_zar)
    values
