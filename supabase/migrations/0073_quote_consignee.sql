-- ============================================================
-- 0073  Consignee / Delivery Point on the quotation
-- ============================================================
-- Adds a Consignee/Delivery Point to the quote, alongside the existing
-- Shipper/Exporter -- who the goods are collected from vs. who they're
-- delivered to, which is sometimes a different company than the paying
-- Customer/Importer. Drawn from the same clients list as Customer (a
-- consignee is conceptually a customer-shaped party); left unset, the
-- app/PDF falls back to the quote's own Customer.
--
-- Run in the Supabase SQL editor after 0072. Self-contained & idempotent.
...
drop function if exists public.save_quote(
  ...
  text, date, text, numeric, numeric, jsonb, jsonb, uuid, uuid, text, text
);
