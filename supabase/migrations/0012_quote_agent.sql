-- ============================================================
-- 0012  Agent on a quote
-- ============================================================
-- Adds quotes.agent_id (the clearing / forwarding agent handling the
-- shipment) and widens save_quote to persist it.
--
-- Run in the Supabase SQL editor after 0011. Self-contained & idempotent.

alter table public.quotes
  add column if not exists agent_id uuid references public.agents (id) on delete set null;

-- Replace save_quote (29-arg signature from 0006 -> drop first).
drop function if exists public.save_quote(
  uuid, text, uuid, uuid, text, text, text, text, text, date, text,
  numeric, numeric, text, text, text, text, text, date, date, text, text,
  text, date, text, numeric, numeric, jsonb, jsonb
);
...
begin
  if p_id is null then
    insert into public.quotes
      (reference, client_id, supplier_id, agent_id, mode, commodity, origin,
       destination, delivery_terms, valid_until, status, commercial_value,
       insurance_amount, incoterms, vessel_name, mbl_no, hbl_no, container_no,
       etd, eta, mawb_no, hawb_no, flight_no, flight_date, carrier_name,
       fx_usd_zar, fx_cny_zar)
    values
      (p_reference, p_client_id, p_supplier_id, p_agent_id, p_mode, p_commodity,
       p_origin, p_destination, p_delivery_terms, p_valid_until, p_status,
       p_commercial_value, p_insurance_amount, p_incoterms, p_vessel_name,
       p_mbl_no, p_hbl_no, p_container_no, p_etd, p_eta, p_mawb_no, p_hawb_no,
       p_flight_no, p_flight_date, p_carrier_name,
