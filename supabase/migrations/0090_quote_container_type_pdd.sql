-- ============================================================
-- 0090  Container Type + Provisional Delivery Date on the quotation
-- ============================================================
-- These already exist on the shipment (jobs.container_type, added 0089;
-- jobs.provisional_delivery_date, added 0019) but had no quote-side field
-- to seed them from. Adds them to quotes, widens save_quote to persist
-- them, and has accept_quote carry them onto the job it creates — same
-- pattern as vessel_name/container_no/carrier_name/shipping_line.
--
-- Run in the Supabase SQL editor after 0089. Self-contained & idempotent.

alter table public.quotes add column if not exists container_type text;
alter table public.quotes add column if not exists provisional_delivery_date date;
...
-- ---- accept_quote also carries container_type / provisional_delivery_date
-- onto the job it creates, same as vessel_name/container_no/etc.
