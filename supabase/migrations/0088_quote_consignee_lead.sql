-- ============================================================
-- 0088  Consignee/Delivery Point can also be a not-yet-promoted Lead
-- ============================================================
-- Mirrors the Customer/Importer field's client_id/lead_id pair: previously
-- consignee_id could only point at a real Customer, so a quote whose
-- Customer/Importer was still a Lead had no way to name a different
-- delivery point that was also a Lead. Left unset, the app/PDF falls back
-- to the quote's own Customer, same as before.
--
-- Run in the Supabase SQL editor after 0087. Self-contained & idempotent.

alter table public.quotes
  add column if not exists consignee_lead_id uuid
