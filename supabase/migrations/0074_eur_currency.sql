-- ============================================================
-- 0074  EUR as a 4th quoting currency
-- ============================================================
-- Quote lines were hard-locked to USD/CNY/ZAR. Adds EUR alongside them:
-- a quotes.fx_eur_zar rate (same flat-column pattern as fx_usd_zar/
-- fx_cny_zar, not a generic multi-currency store -- the business quotes
-- in exactly these four), a Settings default, and widens the line
-- currency check constraint.
--
-- Run in the Supabase SQL editor after 0073. Self-contained & idempotent.
...
alter table public.quote_lines add constraint quote_lines_cur_check
  check (cur in ('USD', 'CNY', 'ZAR', 'EUR'));

alter table public.quotes
  add column if not exists fx_eur_zar numeric not null default 0;

alter table public.company_settings
  add column if not exists default_fx_eur_zar numeric not null default 20.00;
