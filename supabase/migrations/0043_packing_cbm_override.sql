-- 0043 Quote builder: manual CBM override on packing-list rows
--
-- The Cubic M (CBM) figure is normally derived from L x W x H, but operators
-- sometimes need to type a measured/agreed CBM instead. Store it as a nullable
-- column: NULL means "derive from dimensions" (the existing behaviour).

alter table public.packing_list_items
  add column if not exists cbm numeric;

-- Re-emit save_quote so the packing insert carries the optional cbm value.
-- (Unchanged from 0030 except for the packing_list_items column list + select.)
