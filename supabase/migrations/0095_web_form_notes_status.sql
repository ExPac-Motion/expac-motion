-- ============================================================
-- 0095  Web forms: readable Notes formatting + default lead status
-- ============================================================
-- Two fixes to submit_web_form() (0037, 0093):
--
-- 1. Every notes-mapped answer was joined as "Label: value" on one line,
--    including a long free-text answer (e.g. a textarea "Cargo Details"
--    field) -- which then ran on for several sentences right after its
--    own colon, on the same line as the field above it. A textarea field
--    now gets a blank line before it, its label on its own line, and the
--    answer on the line(s) below -- short fields stay as "Label: value".
--    (The Leads page also wasn't rendering newlines at all -- separate
--    fix in ViewField, src/pages/crm/LeadsPage.tsx.)
--
-- 2. Every web-form lead landed with no status. Default it to the
--    existing "Contact Page" lead status, looked up by name so this
--    doesn't depend on a hard-coded id.
--
-- Run in the Supabase SQL editor after 0094. Self-contained & idempotent
...
begin
  ...
  select id into v_status_id from public.lead_statuses where name = 'Contact Page' limit 1;
  ...
  loop
    ...
    if v_map = 'company' then
      ...
      if coalesce(v_field->>'type', '') = 'textarea' then
        v_notes := v_notes
          || case when v_notes = '' then '' else E'\n\n' end
          || coalesce(v_field->>'label', 'Field') || ':' || E'\n' || v_val;
      else
        v_notes := v_notes
          || case when v_notes = '' then '' else E'\n' end
  ...
  insert into public.leads (company, contact, email, phone, notes, source, lead_status_id)
  values (
    v_company,
    nullif(v_contact, ''),
    nullif(v_email, ''),
