-- ============================================================
-- 0013  Transporters + Clearing Agents contact books, and their
--       (internal-only) fields on a quote.
-- ============================================================
-- Same shape as agents (0011). Adds quotes.transporter_id and
-- quotes.clearing_agent_id and widens save_quote to persist them.
--
-- Run in the Supabase SQL editor after 0012. Self-contained & idempotent.
...
declare
  t text;
begin
  foreach t in array array['transporters', 'clearing_agents']
  loop
...
-- Replace save_quote (30-arg signature from 0012 -> drop first).
drop function if exists public.save_quote(
  uuid, text, uuid, uuid, uuid, text, text, text, text, text, date, text,
